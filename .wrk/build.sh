...
ADDON_VER=$(< webinterface.ktalk/addon.xml grep "^<addon\s.*\sversion=" | grep -o "[0-9][0-9\.]\+")
...
find repository.eip/addons -name addon.xml | sort | while read -r f;
do
  < "${f}" grep --invert-match "^<?xml" >> repository.eip/addons/addons-tmp.xml
done;
echo "</addons>" >> repository.eip/addons/addons-tmp.xml
